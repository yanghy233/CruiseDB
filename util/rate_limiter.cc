...
// This API allows user to dynamically change rate limiter's bytes per second.
    ...
    void GenericRateLimiter::Request(int64_t bytes, const Env::IOPriority pri,
        ...
        do {
            // 从 IO 优先级队列中选出一个 leader 作为首要请求，进行处理
            bool timedout = false;
...
// 重新填充令牌
    ...
    // 调节速率核心函数
//    调节方式：
//    后台 IO 完全空闲 (0%): 后台 IO 降至最低速率
//    后台 IO 较空闲(<50%): 降低后台 IO 5% 的速率
//    后台 IO 较繁忙(>90%): 提高后台 IO 5% 的速率
//    正常(50%-90%): 保持不变
    Status GenericRateLimiter::Tune() {
        const int kLowWatermarkPct = 50; // 低水位线 50%
        const int kHighWatermarkPct = 90;  // 高水位线 90%
        const int kAdjustFactorPct = 5;  // 调节步长 5%

        // computed rate limit will be in
        // `[max_bytes_per_sec_ / kAllowedRangeFactor, max_bytes_per_sec_]`.
        // 速率范围因子,实际速率将在[max_bytes_per_sec_ / 20, max_bytes_per_sec_]之间
